...
class liGRUfusion(nn.Module):
    def __init__(self, options, inp_dim):
        ...
        # Initialization of hidden layers
        ################################################################################################
        # for the first layer change to QliGRU

        # Activations
        self.act.append(act_fun(self.ligru_act[0]))
        ...
        # Feed-forward connections
        self.wh.append(QuaternionLinearAutograd(current_input, self.ligru_lay[0],bias=add_bias))
        self.wz.append(QuaternionLinearAutograd(current_input, self.ligru_lay[0],bias=add_bias))

        # Recurrent connections
        self.uh.append(QuaternionLinearAutograd(self.ligru_lay[0], self.ligru_lay[0],bias=False))
        self.uz.append(QuaternionLinearAutograd(self.ligru_lay[0], self.ligru_lay[0],bias=False))


        # batch norm initialization
        self.bn_wh.append(QuaternionBatchNorm1d(self.ligru_lay[0]))
        self.bn_wz.append(QuaternionBatchNorm1d(self.ligru_lay[0]))

        #self.ln.append(LayerNorm(self.ligru_lay[i]))

        if self.bidir:
            current_input=2*self.ligru_lay[0]
        else:
            current_input=self.ligru_lay[0]
        #################################################################################################
        # normal liGRU layers

        for i in range(self.N_ligru_lay-1):
            i = i+1
            # Activations
            self.act.append(act_fun(self.ligru_act[i]))
